...
MultiBEST integrates all required stages into one toolkit, from image processing to the final atomistic model in a suitable format for atomistic simulations, including reactive and classical molecular dynamics as well as spin-lattice dynamics, and to representative continuum models with explicit grain boundary regions. The software is designed to run efficiently on both standard workstations and HPC systems, enabling anything from rapid prototyping to large-scale, high-fidelity simulations.
